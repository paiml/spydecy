    ...
    match ast.node_type.as_str() {
        ...
        "FunctionDecl" => {
            ...
            Ok(CHIR::Function {
                ...
                params,
                body,
                storage_class: StorageClass::Static, // Simplified
        ...
        _ => {
            ...
            bail!("Unsupported C AST node type: {}", ast.node_type)
        }
    }
}
...
fn parse_type(type_str: &Option<String>) -> Type {
    match type_str.as_deref() {
        ...
        Some(s) if s.contains("PyListObject") => {
            Type::C(CType::CPython(spydecy_hir::types::CPythonType::PyListObject))
        }
...
mod tests {
    ...
    fn test_parse_basic_types() {
        assert!(matches!(parse_type(&Some("int".to_string())), Type::C(CType::Int)));
        assert!(matches!(parse_type(&Some("void".to_string())), Type::C(CType::Void)));
        assert!(matches!(parse_type(&Some("size_t".to_string())), Type::C(CType::SizeT)));
    }
